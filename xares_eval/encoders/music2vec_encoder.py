"""X-ARES encoder wrapper for music2vec. See hubert_encoder.py for the
shared rationale (unpooled last_hidden_state, reusing the tested adapter).

Note: music2vec is data2vec-family, not JEPA-family — see the 2026-08-09
correction in CLAUDE.md / audio_comp/models/music2vec.py.
"""
...
class Music2VecXaresEncoder(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self._adapter = get_model_class("music2vec")(device="cuda" if torch.cuda.is_available() else "cpu")
        self._adapter.load()
