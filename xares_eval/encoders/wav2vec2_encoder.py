"""X-ARES encoder wrapper for wav2vec 2.0. See hubert_encoder.py for the
...
class Wav2Vec2XaresEncoder(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self._adapter = get_model_class("wav2vec2")(device="cuda" if torch.cuda.is_available() else "cpu")
        self._adapter.load()
