"""X-ARES encoder wrapper for wav2vec2-Conformer. See hubert_encoder.py for
the shared rationale (unpooled last_hidden_state, reusing the tested
adapter)."""
...
class Wav2Vec2ConformerXaresEncoder(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self._adapter = get_model_class("wav2vec2_conformer")(device="cuda" if torch.cuda.is_available() else "cpu")
        self._adapter.load()
...
if __name__ == "__main__":
    ...
    encoder = Wav2Vec2ConformerXaresEncoder()
