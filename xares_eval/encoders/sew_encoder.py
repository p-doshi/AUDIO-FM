"""X-ARES encoder wrapper for SEW. See hubert_encoder.py for the shared
...
class SEWXaresEncoder(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self._adapter = get_model_class("sew")(device="cuda" if torch.cuda.is_available() else "cpu")
        self._adapter.load()
