"""X-ARES encoder wrapper for MERT. See hubert_encoder.py for the shared
rationale (unpooled last_hidden_state, reusing the tested adapter)."""
...
class MertXaresEncoder(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self._adapter = get_model_class("mert")(device="cuda" if torch.cuda.is_available() else "cpu")
        self._adapter.load()
    ...
    def _forward_one_np(self, waveform_np):
        ...
        with torch.no_grad():
            out = self._adapter._model(**inputs, output_hidden_states=False)
