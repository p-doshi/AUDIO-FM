...
- **Stage 1 — Consolidate current findings (cheap, do first, no new extraction needed).** (a) Downstream functional probe on existing embeddings — linear probe + kNN per model per category, matching X-ARES methodology for comparability to published numbers; specifically check whether audio_jepa's cohesion helps or hurts ship-category classification relative to the masked-modeling clusters. (b) Breadth-hypothesis check — reframe from domain+paradigm to training-distribution-breadth (narrow-speech: hubert/wav2vec2; narrow-music: mert/musicfm; broad-mixed: audio_jepa/clap; self-distillation-narrow-music: music2vec) and test whether this explains the existing RSA matrix better; gate: if it doesn't explain better, keep domain+paradigm as the working explanation rather than forcing the reframe into the write-up. (c) Written decision-rule outcome summary (outcome 2, now with breadth reframing + downstream-probe result folded in) — this alone is a publishable floor result.
- **Stage 2 — Curated model roster expansion (engineering-heavy, gap-filling not count-filling).** Add models that test a currently-missing axis, each verified against its primary paper before entering any comparison table (the music2vec lesson applies without exception): reconstruction-target paradigm (Audio-MAE/SSAST), supervised-vs-self-supervised (AST/Whisper encoder), domain-specific-vs-general (Perch/BirdNET or equivalent bioacoustic model — closest available proxy to the underwater-acoustics target), discrete-token-vs-continuous representation (EnCodec-derived). Gate before Stage 3: every new adapter passes the same discipline as the original 7 — smoke-tested on short/edge-case clips (per the audio_jepa kaldi.fbank lesson), checked for dependency-version side effects (per the flash_attn/torch lesson).
- **Stage 3 — Re-run the geometry toolkit at ~11 models.** Full RDM/RSA/CKA/TwoNN sweep on the same fixed probe set; re-test the breadth hypothesis with the new axes; re-run `inspect_geometry.py` per new model individually. **This is the point at which the Datasets & Benchmarks-track floor is fully met.**
